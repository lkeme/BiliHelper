...
class Active
{
    ...
    public static function run($room_id)
    {
        ...
        if (!($actives = self::check())) {
            return;
        }

        self::lottery($actives);
    }
    ...
    private static function check()
    {
        ...
        if (!$total) {
            Log::warning("直播间 [" . self::$room_id . "] C位光环列表为空，放弃抽奖");
            return false;
        }
    ...
    private static function join($raffle_id)
    {
        ...
        if (isset($data['code']) && $raw['code']) {
            Log::error("C位光环 #{$raffle_id} 抽奖加入失败");
            print_r($de_raw);
        } else {
            Log::notice('参与了房间[' . self::$room_id . ']的C位光环[' . $raffle_id . ']抽奖,抽奖状态: ' . $de_raw['msg']);
            self::$lottery_list[] = [
                'raffle_id' => $raffle_id,
    ...
    private static function lottery(array $actives): bool
    {
        $total = count($actives);
        if (empty($actives) || $total <= 0) {
            return false;
        }
        Live::randFloat();
        foreach ($actives as $raffle_id) {
            if ($total < 10) {
                usleep(0.2 * 1000000);
            } else {
                sleep(10 / $total);
        ...
        for ($i = 0; $i < 10; $i++) {
            ...
            switch ($de_raw['data']['status']) {
                case 3:
                    break;
                case 2:
                    $temp_info = '直播间[' . $room_id . ']C位光环[' . $raffle_id . ']获得[' . $de_raw['data']['gift_name'] . 'X' . $de_raw['data']['gift_num'] . ']';
